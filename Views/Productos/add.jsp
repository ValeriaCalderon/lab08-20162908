...
<body>
		...
		<div class="jumbotron boxlogin">
			<form action="/productos/add" method="post" id="flogin" name="Formulario" onsubmit="document.forms['Formulario']['Enviar'].disabled=true;">
				...
				<input type="text" name="modelo" placeholder="" required class="form-control"> 
	...
	(function(){
			...
			var validarNombre = function(e){
				if(formulario.modelo.value.length < 1){
					alert("Llene el campo Modelo.");
					e.preventDefault();
				}
				else if(isNaN(formulario.modelo.value) == false){
					alert("El campo MODELO tiene caracter/es no válidos.");
					e.preventDefault();
				}
				else{
					validarMarca(e);
			...
			var validarMarca = function(e){
				if(formulario.marca.value.length < 1){
					alert("Llene el campo Marca.");
					e.preventDefault();
				}
				else if(isNaN(formulario.marca.value) == false){
					alert("El campo MARCA tiene caracter/es no válidos.");
					e.preventDefault();
				}
				else{
					validarMedida(e);
			...
			var validarMedida = function(e){
				if(formulario.medida.value.length < 1){
					alert("Llene el campo Medida.");
					e.preventDefault();
				}
				else if(isNaN(formulario.medida.value) == false){
					alert("El campo MEDIDA tiene caracter/es no válidos.");
					e.preventDefault();
				}
				else{
					validarPrecio(e);
			...
			var validarPrecio = function(e){
				if(formulario.precio.value.length < 1){
					alert("Llene el campo Precio.");
					e.preventDefault();
				}
				else if(isNaN(formulario.precio.value) == false){
					alert("El campo PRECIO tiene caracter/es no válidos.");
					e.preventDefault();
				}
				else{
					bloqueoBoton(e);
			...
			var validarAro = function(e){
				if(formulario.aro.value.length < 1){
					alert("Llene el campo Aro.");
					e.preventDefault();
				}
				else if(isNaN(formulario.aro.value) == false){
					alert("El campo ARO tiene caracter/es no válidos.");
					e.preventDefault();
				}
				else{
					bloqueoBoton(e);
