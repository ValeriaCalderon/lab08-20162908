...
public class UsersControllerAdd extends HttpServlet {
	...
	public void doPost(HttpServletRequest requ, HttpServletResponse respon) throws IOException, ServletException {
		...
		/*Date birt=new Date();
		try {
			birt = new SimpleDateFormat("yyyy-MM-dd").parse(requ.getParameter("date"));
		...
		try{
			...
			if(cont==0){	
				User  user= new User(requ.getParameter("email"),requ.getParameter("date"),true,gender,idRol,fecha);
